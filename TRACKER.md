...
| Item | Status | Where |
|---|---|---|
| k/l forest sweep: k ∈ {10,50,100,200,500,978} × l ∈ {0,20,50,100,200,400}, dp_quantile 16/32, ε 10/1000, both cohorts, split 1 | running, 288 jobs, about 8 h | `configs/experiments/pgm_forest_sweep.yaml`, `logs/pgm_forest_sweep.log`, `results/pgm_forest_sweep.csv` |
| MAMA-MIA v2 with many-shadow table selection on held-out aux (COMBINED: reference set; BRCA: non-members, flagged optimistic) | running on the structure-sweep targets | `logs/mamamia_v2_shadow.log`, `results/mamamia_v2.csv` (cliques=shadow) |
| PQRS 1/2/3/4-way retry under zCDP (non-DP selection, reference only) | queued behind the shadow run | `configs/experiments/pgm_pqrs_retry.yaml`, `logs/pgm_pqrs_retry.log` |
...
4. **`mia_output/` (8.6 GB, legacy v1 pipeline, Feb–Apr 2026).** Possibly the abstract's original artifacts.  Delete, archive, or keep?
